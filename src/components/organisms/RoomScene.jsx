...
{/* Interactive Objects */}
      <div className="absolute inset-0 z-10">
        {room.objects.map((object, index) => {
          ...
          if (isCollected && object.isCollectible) return null;
          ...
          return (
            ...
            >
              ...
              >
                {object.name}
                {isExamined && (
                  <ApperIcon name="Eye" className="w-3 h-3 ml-1 inline text-accent" />
                )}
              </motion.div>
            </motion.div>
          );
      ...
      {room.puzzles.map((puzzle, index) => {
        ...
        const hasRequiredObjects = puzzle.requiredObjects?.every(objId => 
          gameState?.examinedObjects?.includes(objId) || 
          gameState?.collectedObjects?.includes(objId)
        );

        if (isSolved || !hasRequiredObjects) return null;
      ...
      {room.puzzles.every(p => gameState?.solvedPuzzles?.includes(p.id)) && (
        <motion.div
          initial={{ opacity: 0, scale: 0 }}
          animate={{ opacity: 1, scale: 1 }}
